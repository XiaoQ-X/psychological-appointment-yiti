...
根目录 `deploy/docker-compose.prod.yml` 会构建静态站点镜像，并由同源 `/api` 反向代理到后端。生产入口只绑定本机端口，必须由学校的 HTTPS 反向代理或负载均衡器对外提供服务。

不要在前端环境变量、构建产物或浏览器存储中写入 AppSecret、数据库密码和服务端加密密钥。
