...
提交体验版或正式版前，必须将 `baseUrl` 改为已在微信公众平台登记的正式 HTTPS request 合法域名。正式域名不得使用 IP、HTTP、自签名证书或开发机局域网地址；修改后需重新执行学生端和咨询师端全流程真机回归。
...
- 全国心理援助热线使用 `12356`；立即医疗急救和报警求助分别使用 `120`、`110`。
- 校内心理中心、值班电话和校医院号码必须在 `config/index.js` 的 `emergencyContacts` 中配置。未配置时页面只显示“待学校配置”，不会发起拨号。
